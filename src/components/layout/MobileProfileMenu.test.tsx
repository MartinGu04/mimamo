...
const { MobileProfileMenu } = await import("./MobileProfileMenu");
...
describe("MobileProfileMenu — no app navigation (nav redesign pass)", () => {
  it("a manager sees no אזור מנהל / מרכז התראות entries here -- those live in BottomNav's עוד sheet instead", () => {
    renderWithTheme(<MobileProfileMenu name="נועה מנהלת" isManager={true} avatarUrl={null} userId="user-test-1" />);
    fireEvent.click(screen.getByRole("button", { name: /תפריט פרופיל/ }));
...
describe("MobileProfileMenu — sign-out cleanup wiring (Phase 9B)", () => {
  it("clears this exact user's device-scoped preferences on click", async () => {
    renderWithTheme(<MobileProfileMenu name="דני בדיקה" isManager={false} avatarUrl={null} userId="user-a" />);
    fireEvent.click(screen.getByRole("button", { name: /תפריט פרופיל/ }));
...
describe("MobileProfileMenu — dismiss behavior", () => {
  ...
  it("a pointerdown outside the panel closes it", () => {
    renderWithTheme(
      <div>
        <MobileProfileMenu name="דני בדיקה" isManager={false} avatarUrl={null} userId="user-test-1" />
