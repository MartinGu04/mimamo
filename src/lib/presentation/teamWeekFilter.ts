 ...
 * `"active"` (the default) shows only people with at least one relevant
 * item somewhere in the displayed week; `"all"` shows every eligible
 * regular/reserve supervisor/technician `buildScheduleTeamWeekView` already
 * put on the roster, even with every cell empty. Never a third definition
 * of eligibility -- `buildScheduleTeamWeekView` already decided WHO belongs
 * on the roster at all (item 1's permanent-personnel exclusion); this only
 * decides which of THOSE people are worth showing right now.
 */
...
export function filterTeamWeekPeople(
  ...
  return teamWeek.people.filter((person) => active.has(person.id));
}
